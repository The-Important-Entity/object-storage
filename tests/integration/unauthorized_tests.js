...
const run_tests = async function(dht_dir, obj_dir, download_dir, upload_dir, client, tester, url, num_nodes, id_max){
    ...
    info = "Test GET namespace when client is unauthorized";
    var res = await client.getNamespaceFiles(test_namespace);
    tester.assert(info, res, "Error: Unauthorized!");

    info = "Test PUT namespace when client is unauthorized";
    var res = await client.putNamespace(test_namespace);
    tester.assert(info, res, "Error: Unauthorized!");

    info = "Test DELETE namespace when client is unauthorized";
    res = await client.deleteNamespace(test_namespace);
    tester.assert(info, res, "Error: Unauthorized!");

    info = "Test GET object when client is unauthorized";
    var res = await client.getObject(test_namespace, test_filename, download_dir);
    tester.assert(info, res, "Error: Unauthorized!");

    info = "Test PUT object when client is unauthorized";
    var res = await client.putObject(test_namespace, test_filename, test_filepath);
    tester.assert(info, res, "Error: Unauthorized!");

    info = "Test DELETE object when client is unauthorized";
    var res = await client.deleteObject(test_namespace, test_filename);
    tester.assert(info, res, "Error: Unauthorized!");
